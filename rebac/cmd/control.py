...
# If ../rebac/__init__.py exists, add ../ to Python search path, so that
# it will override what happens to be installed in /usr/(local/)lib/python...
possible_topdir = os.path.normpath(os.path.join(os.path.abspath(sys.argv[0]),
                                   os.pardir,
                                   os.pardir))
if os.path.exists(os.path.join(possible_topdir, 'rebac', '__init__.py')):
...
from rebac.common import config
...
RELOAD_SERVERS = ['rebac-api', 'rebac-registry']
GRACEFUL_SHUTDOWN_SERVERS = ['rebac-api', 'rebac-registry',
                             'rebac-scrubber']
...
def pid_files(server, pid_file):
    ...
    if pid_file:
        ...
        if os.path.exists('/var/run/rebac/%s.pid' % server):
            pid_files = ['/var/run/rebac/%s.pid' % server]
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(servers=['rebac-' + s for s in ALL_SERVERS])
