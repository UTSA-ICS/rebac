...
opts = [
    ...
    cfg.IntOpt('keystone_auth_port', default=5000),
    cfg.StrOpt('rebac_auth_host', default='10.0.0.43'),
    cfg.IntOpt('rebac_auth_port', default=5454),
...
rebac_auth_host = CONF.authtoken['rebac_auth_host']
rebac_auth_port = CONF.authtoken['rebac_auth_port']

# Copy enforce for 1 method to check access check fpor file
def authorize(context, source_user_info, target_file_info, ):
        ...
        headers = {'X-Auth-Token': context, 'X-Action': "", 'X-Target': ""}
	auth_data = {"sourcefilename": target_file_info, "username":source_user_info}
	print("My auth data is ->", auth_data)
	req = RESTConnect()
        response, data = req._json_request(rebac_auth_host, rebac_auth_port, 'GET', 
	                                    '/rebac/authorize_access', body=auth_data, additional_headers=headers)

	print ("My data is --> ", data)
	print ("My response is --> ", response.read)
	print ("My response is --> ", response.status)
	print ("My response is --> ", response.msg)
	print ("My response is --> ", response.reason)
...
class RESTConnect(object):
    def __init__(self):
            ...
            self.rebac_auth_host = self._conf_get('rebac_auth_host')
            self.rebac_auth_port = int(self._conf_get('rebac_auth_port'))
